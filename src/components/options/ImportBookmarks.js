...
import {
  Anchor, Box, Button, Heading, Text, Meter,
} from 'grommet';
...
class ImportBookmarks extends PureComponent {
  ...
  handleChangeFile = async (event, callback) => {
    ...
    reader.readAsText(event, (elm) => {
    });
  }

  importReaderCallback = (arrCat, arrL) => {
    let count = 0;
    for (let i = 0; i < arrL.length; i += 1) {
      count += arrL[i].length;
    }
    this.setState({ arrCategories: arrCat, arrLinks: arrL, totalLinks: count });
    const { arrCategories } = this.state;
    this.processCategories(arrCategories);
  }
  ...
  postLinks = async (link, label, index) => {
    ...
    const data = {
      link, label, categories_id: index, users_id: user.userid,
    };
  ...
  render() {
    const {
      importStarted, uploadData, uploadError, showErrors,
    } = this.state;
      ...
      <Box align="center" fill className="tabs" style={{ paddinLeft: '4px !important', paddinRight: '4px !important' }}>
        ...
        <Box pad="none">
          ...
          <input
            type="file"
            style={{
              opacity: '0', position: 'relative', margin: 0, bottom: '21px', height: '21px',
            }}
            accept="text/html"
