...
import {
  Anchor, Box, Heading, Text,
} from 'grommet';
import { Rss } from 'grommet-icons';
import { connect } from 'react-redux';
import {
  setListsData, setLinksData, setShowAlertStatus, setPublicUsername,
} from '../../store/Actions';
...
class PublicListCategories extends PureComponent {
  ...
  render() {
    const {
      lists, colNumber, links, bodyWidth, publicPageUsername,
    } = this.props;
      ...
      <Box fill margin="small">
        ...
        <MasonryLayout bodyWidth={bodyWidth} columns={colNumber} gap={10}>
          {lists && lists.length > 0 && lists.map((cat, i) => (
            <Box key={cat.id} pad="xsmall" border={{ side: 'all' }} elevation="small">
              <Box direction="row" justify="between">
                <Heading margin="xxsmall" level="4" textAlign="center" color="brand">{cat.title}</Heading>
              </Box>
              {links && links.length > 0 && links.map((e) => {
                if (e.categories_id === cat.id) {
                  return <PublicLinks key={e.id} id={e.id} link={e.link} label={e.label} margin="xxsmall" />;
                }
                return null;
              })}
            </Box>
